...
* Modified on 9/4/2014 -- Changed "const T& inspectTop() const" to "bool inspectTop(const T& item) const".
...
public:
	...
	// successful
	virtual bool addItem(const T& item) = 0;
	...
	virtual bool removeTop(T& returnValue) = 0;
	...
	virtual bool find(T& returnValue) const = 0;
	...
	virtual bool inspectTop(T& item) const = 0;

	// empties the bag
	virtual void emptyBag() = 0;

	// use the += operator to add an item to the bag
	virtual bool operator+=(const T& addend) = 0;
